...
class Game {
    ...
    guessWord(word, information) {
        this.gameState.update(word, information);
    }
...
class GameState {
    ...
    lintWords() {
        let newValidWordList = this.validWords.slice();
        for (let i = 0; i < this.validWords.length; i++) {
            const word = this.validWords[i];
            if (!this.validateWord(word)) {
                const index = newValidWordList.indexOf(word);
                if (index > -1) {
                    newValidWordList.splice(index, 1);
                }
            }
        }
        this.validWords = newValidWordList;
    };
